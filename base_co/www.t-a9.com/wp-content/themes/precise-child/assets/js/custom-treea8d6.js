  ...
  var init = function () {
    ...
    if (props.searchable) {
      ...
      if (sidebarHeader.length > 0) {
        sidebarHeader[0].prepend(search_parent);
      } else {
        props.parent.prepend(search_parent);
      }
      
      search_parent.innerHTML = `<input type="text" name="menu-tree-search" id="menu-tree-search" class="form-control" placeholder="${props.searchPlaceholderText}"><p class="no-results-tree">No result found.</p>`;
